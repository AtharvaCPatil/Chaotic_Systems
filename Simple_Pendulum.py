...
def sp(i):  # function for animation
    ...
    # plotting ax1 [pendulum animation]
    ax1.set_title("PENDULUM ANIMATION")
    ax1.plot([0, x[len(x) - 1]], [0, y[len(y) - 1]])  # rod
    ax1.plot(x[len(x) - 1], y[len(y) - 1])
    ax1.set_xlim(-l - (0.5 * l), l + (0.5 * l))
    ax1.set_ylim(-l - (0.5 * l), 0)
    if len(t) > 5:
        x_path = [x[len(x) - 1], x[len(x) - 2], x[len(x) - 3], x[len(x) - 4], x[len(x) - 5]]
        y_path = [y[len(y) - 1], y[len(y) - 2], y[len(y) - 3], y[len(y) - 4], y[len(y) - 5]]
...
ani = FuncAnimation(fig, sp, interval=5)
plt.show()
plt.tight_layout()
plt.axis("equal")
